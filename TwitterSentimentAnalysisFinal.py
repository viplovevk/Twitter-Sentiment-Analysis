...
class socketTrigger:
	def __init__(self, s, host, port, keyword):
		self.s = s
		self.host = host
		self.port = port
		self.keyword = keyword
		
	def _socketTrigger(self):
		...
		with c:
			...
			twitter_stream.filter(track=[self.keyword], languages=["en"])
			while True:
				data = c.recv(1024)
				if not data:
					break
					
	def socketTrigger(self):				
		threading.Thread(target=self._socketTrigger, args=[]).start()
...
class preprocessing:
	...
	def preprocessRegex(self):
		words = self.lines.select(explode(split(self.lines.value, "t_end")).alias("Tweet"))
	...
	#Additional unimplemented function that may help remove 
	# def deEmojify(text):
...
class predictor:
	def __init__(self):
		#load trained tokenizers from external into class variable
		self.PN_data_tokenizer = joblib.load("PN_data_tokenizer.joblib")
		self.RT_data_tokenizer = joblib.load("RT_data_tokenizer.joblib")
		
		#load trained Deep Learning Models from external into class variable
		self.Sentiment_model = keras.models.load_model("PNmodel.h5")
		self.RT_model = keras.models.load_model("RTmodel.h5")

	def SentimentPredictor(self, tweetSentence):
		tw = self.PN_data_tokenizer.texts_to_sequences([tweetSentence])
		tw = pad_sequences(tw, maxlen=200)
		a = self.Sentiment_model.predict(tw)
		confidence = a.round(2).item()
		result = int(a.round().item())
		return result, confidence

	def RTPredictor(self, tweetSentence):
		tw = self.RT_data_tokenizer.texts_to_sequences([tweetSentence])
		tw = pad_sequences(tw, maxlen=200)
		a = self.RT_model.predict(tw)
		confidence = a.round(2).item()
		result = int(a.round().item())
		return result, confidence
...
#Applies the predictor functions which uses the trained DL models to give a prediction column
def applyModel(dataDF):
    index = 0
    predictorClass = predictor()
    for i, j in dataDF.iterrows():
        Sresult, confidence = predictorClass.SentimentPredictor(j[0])
        RTresult, confidenceRT = predictorClass.RTPredictor(j[0])
...
def result():
    ...
    if request.method == "POST":
        ...
        SocketListener = socketTrigger(s, host, port, to_predict)
        SocketListener.socketTrigger()
        ...
        prep = preprocessing(unfiltered_stream)
        df = prep.preprocessRegex()
